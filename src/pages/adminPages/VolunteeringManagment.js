...
const VolunteeringManagement = () => {
  ...
  const deleteVolunteer = async (volunteerId) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/volunteering/${volunteerId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`
        }
  ...
  const saveVolunteer = () => {
    fetch(`${process.env.REACT_APP_API_URL}/volunteering/${editingVolunteer._id}`, {
      method: 'PUT',
      headers: {
    ...
    <div className="container mx-auto mt-8">
      ...
      {editingVolunteer ? (
        <div className="max-w-md mx-auto bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
          <h2 className="text-2xl font-bold mb-4 text-red-500">Edit Volunteer</h2>
          <form
            onSubmit={(e) => {
              e.preventDefault();
              saveVolunteer();
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {volunteers.map((volunteer) => (
                <tr key={volunteer._id} className="hover:bg-gray-100">
                  <td className="px-6 py-4 whitespace-nowrap">{volunteer._id}</td>
                  <td className="px-6 py-4 whitespace-nowrap">{volunteer.title}</td>
                  <td className="px-6 py-4 whitespace-nowrap">{volunteer.description}</td>
